...
def step_success(list_stack, deck, show=False):
    ...
    list_stack - стол карт.
    deck - колода карт;
    show - eсли show=True, печатает промежуточные действия на экран.
    """
...
def install_manual_mod(deck, max_nb_stack=5, backspin=False, show=False):
    """
    Функция позволяет вручную играть в данную карточную игру и возвращает итоговый стол карт.
    show - если show=True, печатает промежуточные действия на экран;
    backspin - если backspin=True, заменяет колоду на подкрученную;
    max_nb_stack - установленное игроком максимальное колличество карт на столе к концу игры;
    deck - колода карт.
    """
    if backspin:

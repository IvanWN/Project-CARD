...
def step_success(list_stack, deck, show=False):
    ...
    item - карта для прыжка в автоигре;
...
def install_auto_mod(deck):
    ...
    set - копия колоды, используемая дабы избежать дефформации основной колоды.
...
def request_jump(list_stack):
    """Функция проверяет корректный ли знак введен для прыжка и возвращает его после проверки."""
...
def install_manual_mod(deck, max_nb_stack=2, backspin=False, show=False):
    ...
    img1, img2 - картинки результата игры;
    bib - копия колоды, используемая дабы избежать дефформации основной колоды.
...
def best_consistent_exchange(deck):
    ...
    w - длина результата при стандартной (без подкрутки) игре;
    qwert - копия колоды, используемая дабы избежать дефформации основной колоды.
    bor - промежуточная колода, необходимая для получения оптимальной;
    colod - оптимальная колода.
    Возвращает оптимальную колоду.
    """
